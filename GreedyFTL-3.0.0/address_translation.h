...
#define Vblock2PblockOfTbsTranslation(blockNo)                                                                    \
    (((blockNo) / (USER_BLOCKS_PER_LUN)) * (TOTAL_BLOCKS_PER_LUN) +                                               \
     ((blockNo) % (USER_BLOCKS_PER_LUN))) // Tbs = Total block space
#define Vblock2PblockOfMbsTranslation(blockNo)                                                                    \
    (((blockNo) / (USER_BLOCKS_PER_LUN)) * (MAIN_BLOCKS_PER_LUN) +                                                \
     ((blockNo) % (USER_BLOCKS_PER_LUN))) // Mbs = Main block space
